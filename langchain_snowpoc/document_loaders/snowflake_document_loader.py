...
if __name__ == "__main__":
    ...
    STAGE_NAME = f"{DATABASE_NAME}.PUBLIC.DOCUMENTS"
    CREATE_DB = f"CREATE DATABASE IF NOT EXISTS {DATABASE_NAME}"
    CREATE_DTAGE = f"CREATE STAGE IF NOT EXISTS {STAGE_NAME}"
    ...
    GITHUB_URL = (
        f"{GITHUB_ROOT}"
        "/blob"
        "/9b1369fe6198ee62e0fc8d879209bc4c0b5b9cb6"
        "/site"
        "/sfguides"
        "/src"
        f"/{GITHUB_ASSETS}"
    )
    BIKE_GITHUB_URL = f"{GITHUB_URL}/Mondracer_Infant_Bike.pdf",
